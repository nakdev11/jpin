package kadai210114.ex4_3.article;
...
public class RentalApartmentList {
    ...
    public boolean loadRentalApartment() {
    	try {
    		quantityCount = RentalApartmentDataAccessor.load(apartments);
    		return true;
    ...
    public RentalApartmentList findRentalApartmentByPrice(int upperLimit, int lowerLimit) {
        // 1.6.4.1
        RentalApartmentList targetApartList = new RentalApartmentList();

        for (int i = 0; i < quantityCount; i++) {
        	if (apartments[i].getPrice() >= lowerLimit && apartments[i].getPrice() <= upperLimit) {
        		// 1.6.4.2
        		targetApartList.addRentalApartment(apartments[i]);
        	}
        }
        return targetApartList;
    }
