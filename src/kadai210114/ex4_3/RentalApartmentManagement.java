package kadai210114.ex4_3;

import kadai210114.commons.utility.StandardInputReader;
import kadai210114.ex4_3.article.RentalApartment;
import kadai210114.ex4_3.article.RentalApartmentList;
...
class RentalApartmentManagement {
    ...
    public static void main(String[] args) {
		...
		displayRentalApartment(apartmentList);
		// 1.6
		searchRentalApartment(apartmentList);
    ...
    private static void searchRentalApartment(RentalApartmentList apartmentList) {
    	// 1.6.1
    	System.out.println("希望の賃料範囲で物件検索します");
    	// 1.6.2
    	int upperLimit = StandardInputReader.getInputInt("賃料の上限を入力してください:");
    	// 1.6.3
    	int lowerLimit = StandardInputReader.getInputInt("賃料の下限を入力してください:");
    	// 1.6.4
    	RentalApartmentList targetApartList = apartmentList.findRentalApartmentByPrice(upperLimit, lowerLimit);
    	if (targetApartList.getQuantityCount() != 0) {
    		// 1.6.5
    		displayRentalApartment(targetApartList);
    	} else {
    		// 1.6.6
    		System.out.println("希望の賃貸物件は見つかりませんでした");
    	}
